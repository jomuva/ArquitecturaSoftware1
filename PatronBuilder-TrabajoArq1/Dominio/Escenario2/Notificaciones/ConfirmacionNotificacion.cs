using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace Dominio.Escenario2
{
    public sealed class ConfirmacionNotificacion(INotificacionPlataforma plataforma, string Titulo, string details) : Notificacion(plataforma)
    {
        private readonly string _Titulo = Titulo;
        private readonly string _details = details;

        protected override NotificacionCargarRegistro  ConstruirRegistroMostrar() =>
            new(
                Titulo: _Titulo,
                Cabecera: _details,
                Category: "Confirmación",
                Severidad: Severidad.Suceso,
                Tiempo: DateTimeOffset.UtcNow
            );
    }
}